...
fn can_move_b(robot : (i32,i32), d : char, walls : &HashSet<(i32,i32)>, crates_l : &HashSet<(i32,i32)>, crates_r : &HashSet<(i32,i32)>) -> bool {
    ...
    if walls.contains(&new_pos) {
        ...
        if (d == 'v') || (d == '^') {
            let move_l : bool = can_move_b(new_pos, d, &walls, &crates_l, &crates_r);
            let move_r : bool = can_move_b((new_pos.0,new_pos.1+1), d, &walls, &crates_l, &crates_r);
            return move_l && move_r;
        } else {
            return can_move_b(new_pos, d, &walls, &crates_l, &crates_r);
        }
    } else if crates_r.contains(&new_pos) {
        if (d == 'v') || (d == '^') {
            let move_l : bool = can_move_b((new_pos.0,new_pos.1-1), d, &walls, &crates_l, &crates_r);
            let move_r : bool = can_move_b(new_pos, d, &walls, &crates_l, &crates_r);
            return move_l && move_r;
        } else {
            return can_move_b(new_pos, d, &walls, &crates_l, &crates_r);
        }
...
fn main() -> std::io::Result<()>{
    ...
    for inst in instructions {
        for d in inst {
            ...
            if can_move_b(robot_b, d, &walls_b, &crates_b_l, &crates_b_r) {
